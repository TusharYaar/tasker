...
const Sidebar = ({projects}) => {
    ...
    const projectLinks = projects.map(project => {return <NavLink className="my-2 text-lg tracking-widest  " exact={true} activeClassName="font-bold" key={project.id} to={`/${project.id}`}>{project.projectName}</NavLink>});
